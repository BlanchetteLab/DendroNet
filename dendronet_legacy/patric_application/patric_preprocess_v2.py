...
if __name__ == '__main__':
    ...
    parser.add_argument('--antibiotics', type=str, default=['clindamycin'], metavar='A',
    ...
    parser.add_argument('--feature-folder', type=str, default='data_files/sp_genes', metavar='F',
                        help='name of folder containing feature data(data_files/sp_genes_reduced)')
    parser.add_argument('--label-folder', type=str, default='data_files/genome_ids', metavar='L',
    ...
    parser.add_argument('--output-file', type=str, default='clindamycin_firmicutes_samples.csv', metavar='O',
                        help='name of the output file (default: multiclass_firmicutes_samples.csv)')
    ...
    for file in os.listdir(args.label_folder):
        if file.split('.')[0] in args.antibiotics:
            ...
            genome_ids = genome_ids.union(set(df['genome.genome_id']))
    ...
    for gen_id in genome_ids:
        ...
        for ab_index in range(len(args.antibiotics)):
            ...
            ab_phenotypes = pd.read_csv(os.path.join(args.label_folder, str(curr_ab + '.txt')), dtype=str, sep='\t')
            for index, row in ab_phenotypes.iterrows():
                if row['genome.genome_id'] == gen_id:
                    sample[3][ab_index] = True  # magic number for annotation vector
                    if row['genome_drug.resistant_phenotype'] == 'Resistant':  # or row['genome_drug.resistant_phenotype'] == 'Non-susceptible' todo: decide how to handle this
